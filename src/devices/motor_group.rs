...
impl MotorGroup {
    ...
    pub fn set_voltage_all(&mut self, voltage: f64) -> Vec<Result<(), MotorError>> {
        self.motors
            .iter_mut()
            .map(|motor| motor.set_voltage(voltage))
            .collect()
    }
    pub fn set_voltage_all_for_types(
        &mut self,
        voltage_v5: f64,
        voltage_exp: f64,
    ) -> Vec<Result<(), MotorError>> {
        self.motors
            .iter_mut()
            .map(|motor| {
                motor.set_voltage(match motor.motor_type() {
    ...
    pub fn set_velocity_all(&mut self, rpm: i32) -> Vec<Result<(), MotorError>> {
        self.motors
            .iter_mut()
            .map(|motor| motor.set_velocity(rpm))
            .collect()
    }
